...
class AIModelInterface:
    ...
    local_hf_model: Optional[FinMA7BLocal] = None 
    ...
    def create_from_env(preferred: Optional[AI_MODEL] = None, rate_cache: Optional[RateLimitCache] = None) -> Optional["AIModelInterface"]:
        ...
        for p in provider_order:
            ...
            if p == AI_MODEL.HUGGINGFACE and not rate_cache.is_cached(p.name):
                # Use local FinMA7B implementation instead of HF API
                local_hf = FinMA7BLocal(
                    model_name="microsoft/Phi-3-mini-4k-instruct"
                )
                return AIModelInterface(
                    provider=p,
                    provider_order=provider_order,
                    api_key=os.getenv("HUGGINGFACE_API_KEY"),  # keep key for potential cloud fallback
                    model_name=os.getenv("AI_MODEL_NAME", "microsoft/Phi-3-mini-4k-instruct"),
                    timeout=int(os.getenv("AI_TIMEOUT", "20")),
                    rate_cache=rate_cache,
                    local_hf_model=local_hf   # pass the local model instance
                )
    ...
    def _call_huggingface(self, prompt: str) -> Optional[str]:
        if self.local_hf_model:
            try:
                return self.local_hf_model.analyze(prompt)
            except Exception as e:
                logger.logMessage(f"[huggingface-local] failed: {e}; falling back to cloud API if available.")
                # optionally fall back to API if api_key exists
