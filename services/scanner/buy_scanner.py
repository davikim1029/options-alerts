...
def run_buy_scan(stop_event, consumer=None, caches=None, seconds_to_wait=0, debug=False):
    ...
    for idx, ticker in enumerate(ticker_keys[start_index:], start=start_index):
        ...
        if processedCounter % 200 == 0:
            logger.logMessage(f"Processed {processedCounter} tickers. {len(ticker_keys)-counter} remaining.")
            ...
            for opt in options:
                ...
                for primary in buy_strategies["Primary"]:
                    ...
                    if not success:
                        should_buy = False
                        if debug:
                            logger.logMessage(f"[Buy Scanner] {opt.display} fails {primary.name}: {error}")
